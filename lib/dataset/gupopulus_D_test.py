...
class GUPOPULUS_D_TestDataset(Dataset):
    def __init__(self, data_dir, downscalefactor, D_index):
        """
        ...
        """
        self.hr_dir = Path(data_dir) / f"gupopulus_D{D_index}" / "gupopulus_valid_HR" / f"x{downscalefactor}"
        self.lr_dir = Path(data_dir) / f"gupopulus_D{D_index}" / "gupopulus_valid_LR"
        ...
        for hr_image in self.hr_images:
            self.lr_images.append(hr_image)
