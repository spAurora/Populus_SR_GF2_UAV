...
class Trainer:
    def __init__(self, options):
        ...
        print('option train image size', self.options.train.train_image_size)
            ...
            if to_train_model:
                ...
                if not self.options.train.finetune_percep:
                    if not self.mask_mode:
                        ...
                        ori_loss = ori_loss.mean()
                        statistics["train loss-mask"] = ori_loss.item()
    ...
    def evaluate_one(self, epoch):
        ...
        for i, sample in enumerate(
            ...
            # # 定义自定义文件名 wHy 241009
            # image_filename = os.path.join('./temp', f"epoch_{epoch}_image_{i}.png")
            # image_lr_filename = os.path.join('./temp', f"epoch_{epoch}_image_lr_{i}.png")

            # # 将 Tensor 转换为 PIL 格式，并保存为 PNG
            # transform_to_pil = transforms.ToPILImage()
            # image = image[0,:,:,:]
            # image_lr = image_lr[0,:,:,:]
            # print(image.shape, image_lr.shape)
            # image_pil = transform_to_pil(image.squeeze(0).cpu())  # 去除 batch 维度并迁移到 CPU
            # image_lr_pil = transform_to_pil(image_lr.squeeze(0).cpu())

            # # 保存图像
            # image_pil.save(image_filename)
            # image_lr_pil.save(image_lr_filename)
    ...
    def test_one(self, epoch=None, save_images=False):
        ...
        for sample in tqdm(
            ...
            print('gen, img_hr, img_lr:', gen_diff.shape, image.shape, image_lr.shape)
